...
class Application(Base, TimestampMixin):
    ...
    platform: Mapped[str | None] = mapped_column(
        String(50),
        comment="linkedin | greenhouse | workday | lever | manual"
    )
    ...
    git_path: Mapped[str] = mapped_column(
        String(512),
        comment="Path in GitHub repo: applications/google-swe-2025-02-09/"
    )
    ...
    similarity_score: Mapped[float | None] = mapped_column(
        Float,
        comment="Cosine similarity to most similar previous application (0.0-1.0)"
    )
    ...
    rewrite_strategy: Mapped[str | None] = mapped_column(
        String(50),
        comment="slight_tweak | moderate | ground_up"
    )
    ...
    changes_summary: Mapped[str | None] = mapped_column(
        Text,
        comment="Human-readable summary of what the LLM changed"
    )
