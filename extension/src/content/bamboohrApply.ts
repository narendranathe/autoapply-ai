 ...
 * bamboohrApply.ts
 *
 * Injects an "⚡ AutoFill" button into BambooHR application forms.
 * BambooHR (bamboohr.com) is used by many mid-size companies.
 *
 * DOM landmarks:
 *   - Form container: .ApplicationForm, [data-application-form],
 *                     #applicationForm, form[class*='Application']
 *   - Labels: label[for], aria-label, [class*='fab-Label']
 */
...
function getLabel(el: HTMLElement): string {
  ...
  // BambooHR uses .fab-Label and [class*='fab-Label']
  let parent = el.parentElement;
  for (let i = 0; i < 6; i++) {
    if (!parent) break;
    const lbl = parent.querySelector("label, legend, [class*='fab-Label'], [class*='Label']");
...
const TOAST_ID = "__aap_bhr_toast__";
...
function getFormRoot(): HTMLElement | null {
  return (
    document.querySelector<HTMLElement>(".ApplicationForm") ??
    document.querySelector<HTMLElement>("[data-application-form]") ??
    document.querySelector<HTMLElement>("#applicationForm") ??
    document.querySelector<HTMLElement>("form[class*='Application']") ??
...
const BTN_ID = "__aap_bhr_autofill__";
...
async function runFill(btn: HTMLButtonElement): Promise<void> {
  ...
  if (!root) {
    showToast("⚠ Could not find BambooHR form.", "err");
    btn.disabled = false;
    btn.innerHTML = "&#9889; AutoFill";
    return;
  }
  ...
  // BambooHR: company is in subdomain — company.bamboohr.com
  const hostMatch = window.location.hostname.match(/^([^.]+)\.bamboohr\.com$/);
  const company = hostMatch ? hostMatch[1].replace(/-/g, " ") : document.title.split(/[-–|]/)[0].trim();
  const h1 = document.querySelector<HTMLElement>("h1, [class*='job-title'], [class*='jobTitle']");
  const roleTitle = h1?.textContent?.trim() ?? document.title;
  const descEl = document.querySelector<HTMLElement>("[class*='description'], [class*='jobDescription']");
...
function isBambooHRPage(): boolean {
  return /bamboohr\.com/.test(window.location.href);
}

let _observing = false;

export function initBambooHRApply(): void {
  if (!isBambooHRPage()) return;
  ...
  const observer = new MutationObserver(() => {
    if (!isBambooHRPage()) { removeButton(); return; }
    if (getFormRoot()) injectButton();
    else removeButton();
  });
