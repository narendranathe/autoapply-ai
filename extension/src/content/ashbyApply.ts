 ...
 * ashbyApply.ts
 *
 * Injects an "⚡ AutoFill" button into Ashby application forms.
 * Ashby (jobs.ashbyhq.com / app.ashbyhq.com) uses a single-page
 * application form with standard HTML inputs, selects, and textareas.
 *
 * DOM landmarks:
 *   - Form container: [data-testid="application-form"],
 *                     .ashby-application-form, form.application-form
...
const TOAST_ID = "__aap_ash_toast__";
...
function getFormRoot(): HTMLElement | null {
  return (
    document.querySelector<HTMLElement>("[data-testid='application-form']") ??
    document.querySelector<HTMLElement>(".ashby-application-form") ??
...
const BTN_ID = "__aap_ash_autofill__";
...
async function runFill(btn: HTMLButtonElement): Promise<void> {
  ...
  if (!root) {
    showToast("⚠ Could not find Ashby form. Try scrolling to the form.", "err");
    btn.disabled = false;
    btn.innerHTML = "&#9889; AutoFill";
    return;
  }
  ...
  for (const input of root.querySelectorAll<HTMLInputElement>(
    ...
    const val = matchLabel(getLabel(input, root), profile);
  ...
  for (const sel of root.querySelectorAll<HTMLSelectElement>("select")) {
    ...
    const val = matchLabel(getLabel(sel, root), profile);
  ...
  for (const ta of root.querySelectorAll<HTMLTextAreaElement>("textarea")) {
    ...
    const question = getLabel(ta, root);
...
function extractCompany(): string {
  // jobs.ashbyhq.com/CompanyName or app.ashbyhq.com
...
function extractRole(): string {
  const h1 = document.querySelector<HTMLElement>("h1, [class*='jobTitle'], [class*='job-title']");
  return h1?.textContent?.trim() ?? document.title;
}

function extractJd(): string {
  const el = document.querySelector<HTMLElement>(
    "[class*='description'], [class*='jobDescription'], [data-testid*='description']"
  );
...
function isAshbyPage(): boolean {
  return /ashbyhq\.com/.test(window.location.href);
}

let _observing = false;

export function initAshbyApply(): void {
  if (!isAshbyPage()) return;
  ...
  const observer = new MutationObserver(() => {
    if (!isAshbyPage()) { removeButton(); return; }
    if (getFormRoot()) injectButton();
    else removeButton();
  });
