 ...
 * leverApply.ts
 *
 * Injects an "⚡ AutoFill" button into Lever application forms.
 * Lever (lever.co) is used by Coinbase, Cruise, and many startups.
 *
 * DOM landmarks:
 *   - Form container: .application-form, #application-form,
 *                     [data-form-type="application"]
 *   - Standard HTML inputs, selects, and textareas
 */
...
const LABEL_MAP: Array<{ patterns: RegExp[]; getValue: (p: Profile) => string }> = [
  ...
  { patterns: [/years.*exp/i],                          getValue: (p) => p.yearsExperience },
  { patterns: [/salary/i, /compensation/i],             getValue: (p) => p.salary },
  { patterns: [/sponsor/i, /visa/i, /work\s*auth/i],   getValue: (p) => p.sponsorship?.toLowerCase().includes("no") ? "No" : "Yes" },
];
...
const TOAST_ID = "__aap_lv_toast__";
function showToast(msg: string, type: "ok" | "err" | "info" = "info"): void {
  ...
  if (!toast) {
    ...
    Object.assign(toast.style, {
      ...
      boxShadow: "0 2px 12px rgba(0,0,0,0.3)", transition: "opacity .3s",
...
function getFormRoot(): HTMLElement | null {
  return (
    ...
    document.querySelector<HTMLElement>("#application-form") ??
    document.querySelector<HTMLElement>("[data-form-type='application']") ??
...
const BTN_ID = "__aap_lv_autofill__";
...
function injectButton(): void {
  ...
  Object.assign(btn.style, {
    position: "fixed", bottom: "80px", right: "16px", zIndex: "2147483646",
    background: "linear-gradient(135deg,#6d28d9 0%,#4f46e5 100%)", color: "#fff",
    border: "none", borderRadius: "22px", padding: "10px 18px", fontSize: "13px",
    fontWeight: "700", fontFamily: "system-ui,sans-serif", cursor: "pointer",
    boxShadow: "0 2px 16px rgba(109,40,217,0.5)", letterSpacing: "0.03em",
...
async function runFill(btn: HTMLButtonElement): Promise<void> {
  ...
  ).filter(([, cfg]) => !!cfg.enabled || !!cfg.apiKey).map(([name, cfg]) => ({ name, model: cfg.model }));

  const root = getFormRoot();
  if (!root) {
    showToast("⚠ Could not find Lever form.", "err");
    btn.disabled = false;
    btn.innerHTML = "&#9889; AutoFill";
    return;
  }
  ...
  for (const input of root.querySelectorAll<HTMLInputElement>("input[type=text], input[type=email], input[type=tel], input[type=number], input[type=url]")) {
  ...
  // Extract context from Lever job page
  const h1 = document.querySelector<HTMLElement>("[class*='job-title'], h1");
  const roleTitle = h1?.textContent?.trim() ?? document.title;
  const companyEl = document.querySelector<HTMLElement>("[class*='company-name'], [class*='posting-headline'] h2");
  const company = companyEl?.textContent?.trim() ?? document.title.split(/[-–|]/)[0].trim();
  const descEl = document.querySelector<HTMLElement>("[class*='posting-description'], [class*='section-wrapper']");
  const jdText = (descEl?.textContent ?? "").slice(0, 3000);
...
function classifyQuestion(text: string): string {
  ...
  if (/why.*compan|why.*us/i.test(t)) return "why_company";
...
function isLeverPage(): boolean {
  return /lever\.co/.test(window.location.href);
}

let _observing = false;

export function initLeverApply(): void {
  if (!isLeverPage()) return;
  ...
  const observer = new MutationObserver(() => {
    if (!isLeverPage()) { removeButton(); return; }
    if (getFormRoot()) injectButton();
    else removeButton();
  });
