 ...
 * greenhouseApply.ts
 *
 * Injects an "⚡ AutoFill" button into Greenhouse application forms.
 * Greenhouse (greenhouse.io / boards.greenhouse.io) is used by many
 * tech companies including Airbnb, Dropbox, Pinterest, Robinhood, etc.
 *
 * DOM landmarks:
 *   - Form container: #application_form, [id*='application_form'],
 *                     .application, form.application-form
 *   - Labels: label[for], aria-label, parent label/legend
 */
...
const TOAST_ID = "__aap_gh_toast__";
...
function getFormRoot(): HTMLElement | null {
  return (
    ...
    document.querySelector<HTMLElement>("[id*='application_form']") ??
...
const BTN_ID = "__aap_gh_autofill__";
...
async function runFill(btn: HTMLButtonElement): Promise<void> {
  ...
  if (!root) {
    showToast("⚠ Could not find Greenhouse form.", "err");
    btn.disabled = false;
    btn.innerHTML = "&#9889; AutoFill";
    return;
  }
  ...
  // boards.greenhouse.io/company → extract company from pathname
  const pathMatch = window.location.pathname.match(/^\/([^/]+)/);
  const company = pathMatch
    ? pathMatch[1].replace(/-/g, " ")
    : document.title.split(/[-–|]/)[0].trim();
  const descEl = document.querySelector<HTMLElement>(".content, [class*='job-description']");
...
function isGreenhousePage(): boolean {
  return /greenhouse\.io/.test(window.location.href);
}

let _observing = false;

export function initGreenhouseApply(): void {
  ...
  const observer = new MutationObserver(() => {
    if (!isGreenhousePage()) { removeButton(); return; }
    if (getFormRoot()) injectButton();
    else removeButton();
  });
