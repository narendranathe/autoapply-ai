 ...
 * jobviteApply.ts
 *
 * Injects an "⚡ AutoFill" button into Jobvite application forms.
 * Jobvite (jobs.jobvite.com) is used by many tech companies including
 * Comcast, LinkedIn's ATS platform customers, and others.
 *
 * DOM landmarks:
 *   - Form container: .jv-form, [class*='jv-app-form'], #applicationForm,
 *                     form[id*='jobvite']
 *   - Labels: label[for], aria-label, .jv-field-label
 */
...
function getLabel(el: HTMLElement): string {
  ...
  // Jobvite uses .jv-field-label
  let parent = el.parentElement;
  for (let i = 0; i < 6; i++) {
    if (!parent) break;
    const lbl = parent.querySelector("label, legend, .jv-field-label, [class*='jv-field-label']");
...
const TOAST_ID = "__aap_jv_toast__";
...
function getFormRoot(): HTMLElement | null {
  return (
    document.querySelector<HTMLElement>(".jv-form") ??
    document.querySelector<HTMLElement>("[class*='jv-app-form']") ??
    document.querySelector<HTMLElement>("#applicationForm") ??
    document.querySelector<HTMLElement>("form[id*='jobvite']") ??
...
const BTN_ID = "__aap_jv_autofill__";
...
async function runFill(btn: HTMLButtonElement): Promise<void> {
  ...
  if (!root) {
    showToast("⚠ Could not find Jobvite form.", "err");
    btn.disabled = false;
    btn.innerHTML = "&#9889; AutoFill";
    return;
  }
  ...
  const h1 = document.querySelector<HTMLElement>("h1, .jv-job-title, [class*='job-title']");
  const roleTitle = h1?.textContent?.trim() ?? document.title;
  const descEl = document.querySelector<HTMLElement>(".jv-job-detail-description, [class*='job-description']");
...
function isJobvitePage(): boolean {
  return /jobvite\.com/.test(window.location.href);
}

let _observing = false;

export function initJobviteApply(): void {
  if (!isJobvitePage()) return;
  ...
  const observer = new MutationObserver(() => {
    if (!isJobvitePage()) { removeButton(); return; }
    if (getFormRoot()) injectButton();
    else removeButton();
  });
