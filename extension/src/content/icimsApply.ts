 ...
 * icimsApply.ts
 *
 * Injects an "⚡ AutoFill" button into iCIMS application forms.
 * iCIMS (icims.com) is used by many large enterprises including
 * FedEx, General Motors, T-Mobile, and others.
 *
 * DOM landmarks:
 *   - Form container: [id*='iCIMS_JobsTable'], .iCIMS_Applicant_App_Form,
 *                     #iCIMS_Content_App, form[id*='iCIMS']
 *   - Labels: label[for], aria-label, iCIMS-specific .iCIMS_Label
 */
...
function getLabel(el: HTMLElement): string {
  ...
  // iCIMS uses a preceding .iCIMS_Label or label sibling
  let parent = el.parentElement;
  for (let i = 0; i < 6; i++) {
    if (!parent) break;
    const lbl = parent.querySelector("label, legend, .iCIMS_Label, [class*='iCIMS_Label']");
...
const TOAST_ID = "__aap_ic_toast__";
...
function getFormRoot(): HTMLElement | null {
  return (
    document.querySelector<HTMLElement>("#iCIMS_Content_App") ??
    document.querySelector<HTMLElement>(".iCIMS_Applicant_App_Form") ??
    document.querySelector<HTMLElement>("[id*='iCIMS'][id*='Form']") ??
    document.querySelector<HTMLElement>("[class*='iCIMS'][class*='Form']") ??
    document.querySelector<HTMLElement>("form[action*='icims']") ??
    document.querySelector<HTMLElement>("main form") ??
    document.querySelector<HTMLElement>("form")
  );
}

const BTN_ID = "__aap_ic_autofill__";
...
async function runFill(btn: HTMLButtonElement): Promise<void> {
  ...
  if (!root) {
    showToast("⚠ Could not find iCIMS form.", "err");
    btn.disabled = false;
    btn.innerHTML = "&#9889; AutoFill";
    return;
  }
...
function extractCompany(): string {
  // iCIMS career sites often have the company name in the subdomain: company.icims.com
  const match = window.location.hostname.match(/^([^.]+)\.icims\.com$/);
...
function extractRole(): string {
  const h1 = document.querySelector<HTMLElement>("h1, [class*='job-title'], [class*='jobTitle'], [id*='iCIMS_Header']");
  return h1?.textContent?.trim() ?? document.title;
}

function extractJd(): string {
  const el = document.querySelector<HTMLElement>(
    "[class*='iCIMS_JobContent'], [id*='iCIMS_JobContent'], [class*='description'], [class*='jobDescription']"
  );
...
function isICIMSPage(): boolean {
  return /icims\.com/.test(window.location.href);
}

let _observing = false;

export function initICIMSApply(): void {
  if (!isICIMSPage()) return;
  ...
  const observer = new MutationObserver(() => {
    if (!isICIMSPage()) { removeButton(); return; }
    if (getFormRoot()) injectButton();
    else removeButton();
  });
